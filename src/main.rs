        ...
        .add_paragraph(
            Paragraph::new()
                ...
                .add_hyperlink(
                    ...
                    .add_run(
                        Run::new()
                            .add_text(path.path().file_name().unwrap().to_str().unwrap())
                            .bold()
                            .color("FFFFFF")
...
pub fn add_row_to_table(table: &Table, entry: &std::fs::DirEntry, dir_path: &str) -> Table {
    ...
    if files.len() > 0 {
        ...
        {
            let (last, rest) = relative_path
                .to_str()
                .unwrap()
                .rsplit_once(std::path::MAIN_SEPARATOR)
                .unwrap_or(("", ""));

            t = t.add_row(TableRow::new(vec![
                TableCell::new().add_paragraph(
                    Paragraph::new()
                        ...
                        .add_run(
                            Run::new()
                                .add_text(std::path::MAIN_SEPARATOR)
                                .fonts(RunFonts::new().ascii("Calibri")),
                        )
                        .add_run(
                            Run::new()
            ...
            t = t.add_row(TableRow::new(vec![
                TableCell::new().add_paragraph(
                    Paragraph::new()
                        ...
                        .add_run(
                            Run::new()
                                .add_text(relative_path.to_str().unwrap())
                                .bold()
                                .fonts(RunFonts::new().ascii("Calibri")),
                        ),
                ),
...
fn main() -> Result<(), DocxError> {
    ...
    let path_buf: PathBuf = [&dir_path.to_str().unwrap(), "appendices.docx"]
        .iter()
        .collect();
